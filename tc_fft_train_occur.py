...
def train_occur_for_report():
    ...
    # 获取每个选中组的所有样本
    df_part1_0_selected = df_part1_0.groupby('CUSTOMER_ID').apply(lambda x: x if x.name in selected_groups.values else None).reset_index(drop=True)
    ...
    # 获取每个选中组的所有样本
    df_val = df_part2.groupby('CUSTOMER_ID').apply(lambda x: x if x.name in selected_groups.values else None).reset_index(drop=True)
    ...
    df_val_0 = df_val_0_all.groupby('CUSTOMER_ID').apply(lambda x: x if x.name in selected_groups_0.values else None).reset_index(drop=True)
    ...
    selected_groups_0 = df_train_0_all['CUSTOMER_ID'].drop_duplicates().sample(n=train_0_sample, random_state=train_0_sample) # 3w 1w 5k -> mem leak
    ...
    network.save('./model/'+date_str+'_' + type + '_50_20_16_244_fft_p_t_SS_t' + str(n_line_tail) + '_y16_m01_y23_m01_fl_'+str(train_0_sample)+'_'+str(n_line_back)+'.itc')

    ############################### val
    filename = './result/'+date_str+ '_' + type + '_M6_y16_m1_y23_m1_result_val_' + str(n_line_tail) + '_fl_'+str(train_0_sample)+'_'+str(n_line_back)+'.csv'
    ...
    plt.savefig("./result/"+date_str+"_" + type + "_y16_m1_y23_m01_y23_m7_ROC_val_" + str(n_line_tail) + "_fl_"+str(train_0_sample)+"_"+str(n_line_back)+".png")
    plt.show()
    ...
    plt.legend(loc="lower right")
    plt.savefig("./result/"+date_str +"_" + type + "_y16_m1_y23_m01_y23_m7_KS_val_" + str(n_line_tail) + "_fl_"+str(train_0_sample)+"_"+str(n_line_back)+".png")
    plt.show()

    ############################### test
    filename = './result/'+date_str +'_' + type + '_M6_y16_m1_y23_m1_result_test_' + str(n_line_tail) + '_fl_'+str(train_0_sample)+'_'+str(n_line_back)+'.csv'
    ...
    plt.legend(loc="lower right")
    plt.savefig("./result/"+date_str+"_" + type + "_y16_m1_y23_m1_y23_m7_ROC_test_" + str(n_line_tail) + "_fl_"+str(train_0_sample)+"_"+str(n_line_back)+".png")
    plt.show()
    plt.plot(tpr, lw=2, label='tpr')
    plt.plot(fpr, lw=2, label='fpr')
    plt.plot(tpr - fpr, label='ks')
    ...
    plt.legend(loc="lower right")
    plt.savefig("./result/"+date_str+"_" + type + "_y16_m1_y23_m1_y23_m7_KS_test_" + str(n_line_tail) + "_fl_"+str(train_0_sample)+"_"+str(n_line_back)+".png")
    plt.show()
    ...
    plt.legend(loc="lower right")
    plt.savefig("./result/"+date_str+"_" + type + "_y16_m1_y23_m1_y23_m7_ROC_train_" + str(n_line_tail) + "_fl_"+str(train_0_sample)+"_"+str(n_line_back)+".png")
    plt.show()
    plt.plot(tpr, lw=2, label='tpr')
    plt.plot(fpr, lw=2, label='fpr')
    plt.plot(tpr - fpr, label='ks')
    ...
    plt.legend(loc="lower right")
    plt.savefig("./result/"+date_str+"_" + type + "_y16_m1_y23_m1_y23_m7_KS_train_" + str(n_line_tail) + "_fl_"+str(train_0_sample)+"_"+str(n_line_back)+".png")
    plt.show()
...
def train_occur_for_predict():
    ...
    # 从 0 中 筛选出 33900 个
    selected_groups = df_all_0['CUSTOMER_ID'].drop_duplicates().sample(n=train_0_sample, random_state=train_0_sample- n_line_head)  ## change each
    # 获取每个选中组的所有样本
...
#    exit(0)
    ...
    #network.save('./model/0809_' + type + '_50_20_16_244_fft_p_t_SS_t' + str(n_line_tail) + '_y16_m01_y23_m7_v1.itc')
    #network.save('./model/0815_'+type+'_50_20_16_244_fft_p_t_SS_t'+str(n_line_tail)+'_y16_m01_y23_m01_v1.itc')
    network.save('./model/'+date_str+'_' + type + '_10_10_16_244_fft_p_t_SS_t' + str(n_line_tail) + '_y16_m01_y23_m01_fl_'+str(train_0_sample)+'_'+str(n_line_back)+'.itc')
...
def clean_data_train_occur_continue_for_report():
    ...
    usecol = ['CUSTOMER_ID', 'Y', 'RDATE','XSZQ30D_DIFF','XSZQ90D_DIFF','UAR_AVG_365','UAR_AVG_180','UAR_AVG_90','UAR_AVG_7',
               'UAR_AVG_15','UAR_AVG_30','UAR_AVG_60','GRP_AVAILAMT_SUM','USEAMOUNT_RATIO','UAR_CHA_365','UAR_CHA_15','UAR_CHA_30',
               'UAR_CHA_60','UAR_CHA_90','UAR_CHA_180','UAR_CHA_7','STOCK_AGE_AVG_365','SDV_REPAY_365','INV_AVG_365',
               'GRP_REPAYCARS180_SUM','JH_CCC','JH_HGZ','JH_JTS','LRR_AVG_365','LSR_91_AVG_365','STOCK_AGE_AVG_180',
               'FREESPANRP_360D_R','SDV_REPAY_180','XSZQ180D_R','JH_SC_R','INV_AVG_180','GRP_REPAYCARS90_SUM','GRP_CNT',
...
def augment_bad_data_train_occur_continue_for_report():
    usecols = ['CUSTOMER_ID', 'Y', 'RDATE', 'XSZQ30D_DIFF', 'XSZQ90D_DIFF', 'UAR_AVG_365', 'UAR_AVG_180', 'UAR_AVG_90',
               'UAR_AVG_7', 'UAR_AVG_15', 'UAR_AVG_30', 'UAR_AVG_60', 'GRP_AVAILAMT_SUM', 'USEAMOUNT_RATIO',
               'UAR_CHA_365','UAR_CHA_15', 'UAR_CHA_30', 'UAR_CHA_60', 'UAR_CHA_90', 'UAR_CHA_180', 'UAR_CHA_7', 'STOCK_AGE_AVG_365',
               'SDV_REPAY_365','INV_AVG_365', 'GRP_REPAYCARS180_SUM', 'JH_CCC', 'JH_HGZ', 'JH_JTS', 'LRR_AVG_365', 'LSR_91_AVG_365',
               'STOCK_AGE_AVG_180', 'FREESPANRP_360D_R', 'SDV_REPAY_180', 'XSZQ180D_R', 'JH_SC_R', 'INV_AVG_180',
               'GRP_REPAYCARS90_SUM','GRP_CNT', 'JH_HGZ_R', 'GRP_USEAMT_SUM', 'GRP_REPAYCARS30_SUM', 'STOCK_AGE_AVG_90',
               'LSR_91_AVG_180','STOCK_AGE_AVG_60','XSZQ90D_R','SDV_REPAY_90','INV_AVG_90','LSR_121_AVG_365','FREESPANRP_180D_R','SDV_REPAY_60',
               'LRR_AVG_180','INV_AVG_60','STOCK_AGE_AVG_30','JH_180_CNT','INV_AVG_30','STOCK_AGE_AVG_15','XSZQ30D_R','STOCK_AGE_AVG_7','SDV_REPAY_30',
               'LSR_91_AVG_90','STOCK_AGE_CHA_RATIO_7','INV_RATIO_90','STOCK_AGE_AVG','STOCK_AGE_CHA_RATIO_365','STOCK_AGE_CHA_RATIO_180',
               'STOCK_AGE_CHA_RATIO_90','STOCK_AGE_CHA_RATIO_60','STOCK_AGE_CHA_RATIO_30','STOCK_AGE_CHA_RATIO_15','LSR_91_AVG_60',
    ...
    cols = ['XSZQ30D_DIFF', 'XSZQ90D_DIFF', 'UAR_AVG_365', 'UAR_AVG_180', 'UAR_AVG_90', 'UAR_AVG_7',
           ...
           'UAR_CHA_30','UAR_CHA_60', 'UAR_CHA_90', 'UAR_CHA_180', 'UAR_CHA_7', 'STOCK_AGE_AVG_365', 'SDV_REPAY_365', 'INV_AVG_365',
           'GRP_REPAYCARS180_SUM', 'JH_CCC', 'JH_HGZ', 'JH_JTS', 'LRR_AVG_365', 'LSR_91_AVG_365', 'STOCK_AGE_AVG_180',
...
def ts2vec_relabel():
    usecols = ['CUSTOMER_ID', 'Y', 'RDATE', 'XSZQ30D_DIFF', 'XSZQ90D_DIFF', 'UAR_AVG_365', 'UAR_AVG_180', 'UAR_AVG_90',
               ...
               'UAR_CHA_365','UAR_CHA_15', 'UAR_CHA_30', 'UAR_CHA_60', 'UAR_CHA_90', 'UAR_CHA_180', 'UAR_CHA_7']  # 17 cols
    df23 = pd.read_csv("./data/0825_train/occur/2023_202308251939.csv", header=0, usecols=usecols, sep=',',encoding='gbk')
    df22_4 = pd.read_csv("./data/0825_train/occur/2022_10_12_202308250913.csv", header=0, usecols=usecols, sep=',',encoding='gbk')
    df22_3 = pd.read_csv("./data/0825_train/occur/2022_7_10_202308250922.csv", header=0, usecols=usecols, sep=',',encoding='gbk')
    df22_2 = pd.read_csv("./data/0825_train/occur/2022_4_7_202308250927.csv", header=0, usecols=usecols, sep=',',encoding='gbk')
...
def ts2vec_relabel(train_tsdatasets: List[TSDataset],train_labels: np.ndarray,train_cutomers: np.ndarray,):
    ts2vec_params = {"segment_size": 30,
                     "repr_dims": 320,  # 320
                     "batch_size": 128,
                     # "sampling_stride": 200,
    ...
    from paddlets.models.representation import TS2Vec
    model = ReprCluster(repr_model=TS2Vec, repr_model_params=ts2vec_params)
    model.fit(train_tsdatasets)
    y_pred = model.predict(train_tsdatasets)
    print(y_pred, y_pred.sum(), len(y_pred))

    ## prob to csv
    df = pd.DataFrame()
    df['Y'] = train_labels
    df['customerid'] = train_cutomers
    # df.to_csv(filename, index=False)
    # df = pd.read_csv(filename)
    preds_prob = y_pred.tolist()
    new_data = {"prob": preds_prob, }
