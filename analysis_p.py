...
print(dfAlt0.shape)
print(dfAlt1.shape)
print(dfAlt0.tail(48))
print(dfAlt1.tail(48))
...
dfAlt1 = dfAlt1.groupby(['CUSTOMERID']).apply(lambda x: x.sort_values(["RDATE"], ascending = True)).\
    reset_index(drop=True).groupby('CUSTOMERID').tail(24)
print(dfAlt0.head(61))
print(dfAlt1.tail(61))
print(dfAlt0.shape)
print(dfAlt1.shape)
print(dfAlt0.columns)

from paddlets import TSDataset
from paddlets.analysis import FFT,STFT,CWT
...
stft=STFT()
cwt = CWT()
for data in tsdatasets0:
    #res = fft(data)
    #res = stft(data) # zxx 13*3
    res = cwt(data)  # coefs 63*24 complex128 x+yj
    for x in data.columns:
        #print(res[x+"_amplitude"])
        #print(data[x])
        #print(data[x].index)
        #print(res[x+"_amplitude"].index)
        #res[x+"_amplitude"].index = data[x].index
        #res[x + "_phase"].index = data[x].index
        #data[x] = res[x+"_amplitude"]
        #data.set_column(column=x+"_amplitude",value=res[x+"_amplitude"],type='target')
        #data.set_column(column=x + "_phase", value=res[x + "_phase"], type='target')
        print(res[x+"_coefs"][-1])
        coefs = [n for n in range(len(res[x+"_coefs"][-1]))]
        #phase = [n for n in range(len(res[x+"_coefs"][-1]))]
        for i in range(len(res[x+"_coefs"])):
            for j in range(len(res[x+"_coefs"][i])):
                #phase[j] = cmath.phase(res[x + "_coefs"][-1][j])
                coefs[j] = abs(res[x + "_coefs"][-1][j])
            coefs = Series(coefs)
            #phase = Series(phase)
            coefs.index = data[x].index
            #phase.index = data[x].index
            data.set_column(column=x+"_coefs_"+str(i),value=coefs,type='target')
            #data.set_column(column=x + "_phase", value=phase, type='target')
        print(data[x])


#tsdatasets0[0].drop('new_b')
...
for data in tsdatasets1:
    report = AnalysisReport(data, ["fft","stft","cwt"], customized_config,['GRP_REPAYCARS90_SUM', 'AVG_DBDKTC1W_180'])
# export a file named "analysis_report.docx" to current path by default
    strs = str(num) + ".docx"
    report.export_docx_report(file_name=strs)
    num += 1